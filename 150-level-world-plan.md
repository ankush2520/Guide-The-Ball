...
- **Moving targets** (new, Needlecrest) — the target zone slides back and forth horizontally between two bounds at a fixed rate, rather than sitting still. Deliberately horizontal-only (not full 2D) so it stays a solvable timing puzzle rather than tipping into a reflex/luck mechanic — the player plans the ramp layout knowing the target's position is a function of elapsed drop time, same "plan then watch" contract as everything else in the game. The target's path owns a **lane** (the capsule it sweeps, plus a ramp's thickness and a gap) that is carved out of the ramp-placement region and drawn on the board as a track, so a ramp can never sit where the target is about to slide through it — the overlap that got the first version cut. Reusable in any country: see `src/entities/MovingTarget.ts` and `src/levels/patrol.ts`. Emberkeep uses it on two final-exam boards (36, 39).
